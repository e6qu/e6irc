...
| Native-client user | `e6irc`, `e6irc-tui`, or another client using `e6irc-client` | Anonymous, SASL password (SCRAM-SHA-512/256 or PLAIN, the strongest a network offers), or direct/device-cached SASL OAUTHBEARER |
...
| [Web chat](web-chat.md) | enter the application, choose a network, live chat, replay/history, membership state, themes, desktop notifications, disconnect and recovery |
| [Networks and BNC](networks-and-bouncer.md) | enable the registry/listener, add a preset or custom network, connect and diagnose, attach an IRC client, persist/replay, edit/disable/enable/delete |
