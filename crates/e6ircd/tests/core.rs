...
/// with ERR_NOSUCHNICK when nobody holds the nick. A mask longer than a K-line
/// may be is refused without being echoed.
#[test]
fn kline_of_a_nick_bans_its_host() {
    ...
    s.line(op, &format!("KLINE *@{}.example :long", "h".repeat(100)));
    assert_eq!(
        s.drain(op),
        [":irc.test.example NOTICE god :Refusing K-Line: the mask is longer than 100 bytes"]
    );
}
