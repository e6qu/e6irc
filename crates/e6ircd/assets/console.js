(() => {
  ...
  if (adminAccountsPage instanceof HTMLElement) {
    ...
    const renderInvitations = (data) => {
      ...
      if (!rows.length) { invitationHost.append(element("p", "empty", "No pending invitations.")); return; }
      const body = document.createElement("tbody");
      for (const invitation of rows) { const revoke = document.createElement("form"); revoke.className = "cell-form"; revoke.dataset.apiAdminInvitationDelete = ""; revoke.dataset.confirm = `Revoke the invitation for ${invitation.account}?`; revoke.action = `/api/v1/admin/invitations/${encodeURIComponent(invitation.id)}`; revoke.append(capability(), button("Revoke", "danger")); const expires = element("time", "", invitation.expires_at); expires.dateTime = invitation.expires_at; body.append(append(element("tr"), append(element("td"), append(element("strong"), element("code", "", invitation.account))), element("td", "", invitation.contact_email || "Not supplied"), element("td", "", invitation.administrator ? "administrator" : "member"), append(element("td"), element("code", "", invitation.created_by)), element("td", "", expires), append(element("td"), revoke))); }
      invitationHost.append(append(element("div", "scroll"), append(document.createElement("table"), body)));
    };
    const renderAccounts = (data) => {
      ...
      if (!rows.length) { accountHost.append(element("p", "empty", "No account matches this exact name.")); return; }
      const body = document.createElement("tbody");
      for (const account of rows) { const auth = account.authentication || {}; const resources = account.resources || {}; const actions = element("td"); if (account.current) actions.append(element("span", "meta", "Current account")); else { for (const [key, value, label] of [["suspension", !account.suspended, account.suspended ? "Reactivate" : "Suspend"], ["administrator", !account.administrator_sources.durable, account.administrator_sources.durable ? "Revoke durable admin" : "Grant durable admin"]]) { const form = document.createElement("form"); form.className = "cell-form"; form.dataset.apiAdminAccountState = key; form.action = `/api/v1/admin/accounts/${encodeURIComponent(account.id)}`; const state = document.createElement("input"); state.type = "hidden"; state.name = key === "suspension" ? "suspended" : "administrator"; state.value = String(value); form.append(capability(), state, button(label, value ? "" : "danger")); actions.append(form); } } body.append(append(element("tr"), element("td", "meta", account.id), append(element("td"), append(element("strong"), element("code", "", account.name))), element("td", "", auth.local_password ? "local password" : "no local password"), element("td", "", account.suspended ? "suspended" : account.administrator ? "administrator" : "active"), element("td", "", `${auth.browser_sessions} browsers · ${auth.api_tokens} tokens`), element("td", "", `${resources.networks} networks · ${resources.founded_channels} channels`), actions)); }
      accountHost.append(append(element("div", "scroll"), append(document.createElement("table"), body)));
    };
