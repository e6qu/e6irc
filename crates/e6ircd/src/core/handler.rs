...
fn cmd_batch(state: &mut ServerState, conn: ConnId, msg: &Message, p: &[&str]) {
    ...
    match sign {
        "+" => {
            ...
            session.multiline = Some(super::state::MultilineBatch {
...
fn deliver_multiline(state: &mut ServerState, conn: ConnId, batch: super::state::MultilineBatch) {
    ...
    let (hist_key, peers) = match &resolved.kind {
        ResolvedKind::Channel { key, status_prefix } => {
            ...
            (super::state::HistoryKey::from(key), Vec::new())
        }
...
fn tag_prefix(tags: &[String]) -> String {
    if tags.is_empty() {
        String::new()
    } else {
        format!("@{} ", tags.join(";"))
...
fn send_multiline_line(state: &mut ServerState, conn: ConnId, bypass_capture: bool, line: &str) {
...
const MULTILINE_CAP: &str = "draft/multiline";
...
const MULTILINE_MAX_LINES: usize = 32;
/// Tag marking a line as continuing the previous one without a break.
const MULTILINE_CONCAT_TAG: &str = "draft/multiline-concat";
...
/// the on-JOIN replay.
fn send_current_markread(state: &mut ServerState, conn: ConnId, key: &ChanKey, display: &str) {
    let account = state.sessions[&conn].account.clone();
