...
pub(crate) fn channel_command(
    ...
    let result = match command.operation() {
        ...
        crate::core::state::ChannelCommandOperation::ChanServStatus { .. } => {
            Some(crate::core::state::ChannelCommandResult::ChanServStatus(
                services::chanserv_status_on_owner(state, command),
            ))
        }
...
pub(crate) fn channel_command_result(
    ...
    match result {
        ...
        crate::core::state::ChannelCommandResult::ChanServStatus(result) => state
            .emit_deferred_labeled(conn, label, |state| {
                services::emit_chanserv_status_result(state, conn, result)
            }),
...
fn dispatch_parsed(state: &mut ServerState, conn: ConnId, msg: &Message) {
    ...
    match command.as_str() {
        ...
        "KLINE" => cmd_add_ban(state, conn, BanKind::Kline, p, msg.has_trailing),
        "UNKLINE" => cmd_remove_ban(state, conn, BanKind::Kline, p),
        "DLINE" => cmd_add_ban(state, conn, BanKind::Dline, p, msg.has_trailing),
        "UNDLINE" => cmd_remove_ban(state, conn, BanKind::Dline, p),
        "XLINE" => cmd_add_ban(state, conn, BanKind::Xline, p, msg.has_trailing),
