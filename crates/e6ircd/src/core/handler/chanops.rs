...
pub(super) fn pace_channel_list(state: &mut ServerState, conn: ConnId) {
    ...
    while room > 0 {
        match cursor.next_row() {
            NextRow::Row(row) => {
                ...
                send_list_line(state, conn, cursor.batch.as_deref(), line);
                room -= 1;
            }
    ...
    // than the room it has made: one that is not reading holds nothing.
    if let Some(limit) = std::num::NonZeroUsize::new(room) {
