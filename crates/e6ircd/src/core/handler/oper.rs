...
pub(super) enum BanReject {
    ...
    /// The mask is longer than its kind allows (carried): not echoed, since a
    /// mask that long would not fit the refusal either.
    TooLong(usize),
}
...
impl BanReject {
    ...
    pub(super) fn describe(&self, label: &str) -> String {
        match self {
            ...
            Self::TooLong(max) => {
                format!("Refusing {label}: the mask is longer than {max} bytes")
            }
        }
    }
}
...
impl BanMask {
    ...
    pub(super) fn parse(
        ...
        // Bounded like a channel list mask (BANMASKLEN): the mask rides every
        // STATS row, listing and notice, which must fit one line. An X-line
        // matches a realname, which is at most REALLEN, so its mask may be as
        // long; a longer mask of either kind could match nothing it is for.
        let max_len = match kind {
            BanKind::Kline | BanKind::Dline => super::channel::BANMASKLEN,
            BanKind::Xline => super::channel::REALLEN,
        };
        if display.len() > max_len {
            return Err(BanReject::TooLong(max_len));
        }
        match kind {
...
pub(super) fn cmd_add_ban(
    ...
    if p.is_empty() {
        ...
        let lines: Vec<String> = state
            ...
            .map(|b| {
                ...
                fitted_line(
                    format!(
                        ":{server} NOTICE {nick} :{label} {}{temporary} (by {}) :",
                        b.mask.as_str(),
                        b.set_by,
                    ),
                    &b.reason,
    ...
    let (parsed_mask, reason) = match BanMask::parse(kind, &params, has_trailing) {
        ...
        Err(reject) => {
            ...
            let line = fitted_line(format!(":{server} NOTICE {nick} :"), &refusal);
            state.send(conn, &line);
            return;
        }
    };
    ...
    if !state.config.sasl_enabled {
        let text = ban_added_text(kind, mask.as_str(), expiry);
...
pub(crate) fn expire_server_bans(state: &mut ServerState) {
    ...
    let server = state.config.server_name.clone();
    let operators: Vec<(ConnId, String)> = state
        .sessions
        .iter()
        .filter(|(_, session)| session.is_registered() && session.oper.is_some())
        .filter_map(|(&conn, session)| Some((conn, session.nick()?.to_string())))
        .collect();
    for ban in lapsed {
        let text = format!(
            "Temporary {} for {} expired",
            ban.kind.label(),
            ban.mask.as_str()
        );
        for (conn, nick) in &operators {
            let line = fitted_line(format!(":{server} NOTICE {nick} :*** Notice -- "), &text);
            state.send(*conn, &line);
        }
    }
}
