...
mod ingress_tests {
    ...
    #[tokio::test]
    async fn nick_claim_is_shared_by_core_shards() {
        let config = Config {
            name: "two-worker-nicks",
            capacity: 1,
            policy: Policy::Fifo,
        };
        ...
        let shards = CoreShardCount::new(NonZeroUsize::new(2).expect("two shards"));
        let (first_db, _first_db_rx) = queue(Config {
            name: "first-worker-nicks-db",
            capacity: 1,
            policy: Policy::Fifo,
        });
        let (second_db, _second_db_rx) = queue(Config {
            name: "second-worker-nicks-db",
            capacity: 1,
            policy: Policy::Fifo,
        });
        let mut first = Core::with_telemetry_on_shard_with_nicks(
        ...
        let (first_out, _first_output) = queue(Config {
            name: "first-worker-nicks-output",
            capacity: 1,
            policy: Policy::Fifo,
        });
        let (second_out, mut second_output) = queue(Config {
            name: "second-worker-nicks-output",
            capacity: 1,
            policy: Policy::Fifo,
        });
        first.state.open(
        ...
        let reply = second_output.pop().await.expect("nickname-in-use reply");
        assert!(reply.payload.0.starts_with(b":irc.test 433 * ALICE "));
    }
