...
mod tests {
    ...
    async fn shutdown_stops_the_bouncer_drivers_with_a_goodbye() {
        ...
        tokio::spawn(async move {
            ...
            while let Some(line) = lines.next_line().await.unwrap() {
                if line.starts_with("CAP LS") {
                    ...
                    // As a server does, it ends the session by closing the
                    // link, which the driver waits for before its stop is
                    // done. Waiting instead for the driver to close first made
                    // the order of `None` and the end of `run` a race.
                    break;
                }
            }
            // Sent before the socket closes, so before the driver can finish.
        ...
        assert_eq!(
            ...
            "the upstream reads the goodbye, and the session ends, before shutdown completes"
        );
        drop(registry);
    }
