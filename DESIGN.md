...
│   ├── e6irc-queue/          # custom bounded queue: the only inter-subsystem
...
| Queues | **custom `e6irc-queue`** | The single inter-subsystem communication primitive; built in-repo (no channel crates) so it can be step-scheduled, traced, and loom-verified (§7.3). |
...
| Logging | **tracing** + tracing-subscriber | Structured; JSON output option. |
| Metrics | Prometheus text endpoint, behind `metrics` feature | Zero cost when disabled. |
...
| `bridge-matrix` / `bridge-discord` / `bridge-slack` | off | Each bridge driver and its HTTP/WS client code. |
| `metrics` | off | Prometheus endpoint. |
| `openapi` | off | utoipa-generated OpenAPI document at `/api/v1/openapi.json`. |
| `ws-irc` | on | Native IRC-over-WebSocket endpoint (§13.4) — lets third-party web IRC clients (e.g. gamja) connect. |
...
**`e6irc-queue` (custom, in-repo — no channel crates):**
...
- **11.3 Hot path**: per-channel in-memory ring (last 500 events) answers
  the common "LATEST *" without Postgres; misses fall through to the
  `messages` table. **Rings are lazy and LRU-evicted** so hot-history RAM
  is bounded by *activity*, not channel count: only the
  `max_hot_channels` (default 8192) most-recently-active channels hold a
...
- `admin`: accounts, global bans (kline-equivalents), server stats,
  audit-log query
- `healthz` (liveness; no auth), `metrics` (feature-gated, admin-only)

OpenAPI document generated by utoipa behind the `openapi` feature so the
schema types annotate once and the default build pays nothing.
...
- `tracing` spans: connection lifecycle, message routing (sampled), driver
  connects, HTTP requests. Human logs by default, `--log-format json` for
  shipping.
- `metrics` feature: connections by state, messages/s, fan-out latency
  histogram, SendQ kills, Postgres batch latency, per-driver up/down.
- Slow-path visibility: SendQ overflows, PG write-queue depth, and driver
  reconnect storms all surface as WARN + metrics, never silently absorbed.
